...
for cnt in sorted_contours:
    ...
    cv2.imshow("ROI", roi)
    cv2.waitKey(0)
    leitura = str(reader.readtext(roi, detail = 0))
    #caracs = str(leitura)

    text0 = leitura.replace(' ', '')
    caracs = text0.replace("-", "")
    letras = caracs[:3]
    num = caracs[3:]
    num = num.replace("-", "")
    ...
    #plate_num += text1       # <<<<<< Isso que repete a função text0, repete e salva a quantidade de letras e numeros da placa
#print(plate_num)
#print(leitura)
